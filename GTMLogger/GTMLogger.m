...
- (void)logDebug:(NSString *)fmt, ... {
    va_list args;
    va_start(args, fmt);
    [self logInternalFunc:NULL line: -1 format:fmt valist:args level:kGTMLoggerLevelDebug];
    va_end(args);
}
...
- (void)logInfo:(NSString *)fmt, ... {
    va_list args;
    va_start(args, fmt);
    [self logInternalFunc:NULL line: -1 format:fmt valist:args level:kGTMLoggerLevelInfo];
    va_end(args);
}
...
- (void)logError:(NSString *)fmt, ... {
    va_list args;
    va_start(args, fmt);
    [self logInternalFunc:NULL line: -1 format:fmt valist:args level:kGTMLoggerLevelError];
    va_end(args);
}
...
- (void)logAssert:(NSString *)fmt, ... {
    va_list args;
    va_start(args, fmt);
    [self logInternalFunc:NULL line: -1 format:fmt valist:args level:kGTMLoggerLevelAssert];
    va_end(args);
}
...
- (void)logFuncDebug:(const char *)func line:(int)linenum msg:(NSString *)fmt, ... {
    va_list args;
    va_start(args, fmt);
    [self logInternalFunc:func line: linenum format:fmt valist:args level:kGTMLoggerLevelDebug];
    va_end(args);
}

- (void)logFuncInfo:(const char *)func line:(int)linenum msg:(NSString *)fmt, ... { 
    va_list args;
    va_start(args, fmt);
    [self logInternalFunc:func line: linenum format:fmt valist:args level:kGTMLoggerLevelInfo];
    va_end(args);
}

- (void)logFuncError:(const char *)func line:(int)linenum msg:(NSString *)fmt, ... {
    va_list args;
    va_start(args, fmt);
    [self logInternalFunc:func line: linenum format:fmt valist:args level:kGTMLoggerLevelError];
    va_end(args);
}

- (void)logFuncAssert:(const char *)func line:(int)linenum msg:(NSString *)fmt, ... {
    va_list args;
    va_start(args, fmt);
    [self logInternalFunc:func line: linenum format:fmt valist:args level:kGTMLoggerLevelAssert];
    va_end(args);
}
...
- (NSString *)stringForFunc:(NSString *)func
  ...
  return [NSString stringWithFormat:@"%@ %@[%d/%p] [lvl=%d] %@ [Line Number:%@] %@",
          tstamp, pname_, pid_, pthread_self(),
          level, [self prettyNameForFunc:func],
          [NSString stringWithFormat:@"%d", linenum],
          // |super| has guard for nil |fmt| and |args|
          [super stringForFunc:func inLine:linenum withFormat:fmt valist:args level:level]];
}
